::  sovnas-fileserver: generic from-clay file-serving agent
::
::    serves static frontend files from /web in this desk.
::    based on ~paldev's foo-fileserver pattern.
::
...
++  on-poke
  ...
  :-  :-  %&
      ...
      %-  (bond |.(^$(site (snip site))))
      (~(get by auth) site)
  =/  fext=@ta  ?~(ext %html u.ext)
  =.  site  (skip site |=(s=@t =('' s)))
  =.  site  ?~(site ~[%index] site)
  =/  =path
    :*  (scot %p our.bowl)
        q.byk.bowl
        (scot %da now.bowl)
        (weld file-root (snoc site fext))
    ==
  ?.  .^(? %cu path)
    ~&  [dap.bowl %not-found path=path]
    [[404 ~] `(as-octs:mimes:html 'not found')]
  =+  .^(file=^vase %cr path)
  =+  ~|  [%no-mime-conversion from=fext]
      .^(=tube:clay %cc (scot %p our.bowl) q.byk.bowl (scot %da now.bowl) /[fext]/mime)
  =+  !<(=mime (tube file))
  :_  `q.mime
